    ...
    P: AsRef<Path>,
{
    ...
    if let Some(container_tag) = env::var_os("ICEDRAGON_RUNNER") {
        cmd.arg("--runner").arg(&container_tag);
    }
    cmd.arg("--");
    cmd
}
...
{
    let bin_file =
        File::open(&bin_path).expect(&format!("failed to open the binary file {bin_path:?}"));
    let mut bin_content = Vec::new();
    let mut reader = BufReader::new(bin_file);
    reader
        .read_to_end(&mut bin_content)
        .expect(&format!("could not read the binary file {bin_path:?}"));
    let elf =
        Elf::parse(&bin_content).expect(&format!("could not parse the binary file {bin_path:?}"));
...
fn test_cmake_compiler_rt(target: &str, arch: &str, elf_machine: u16, extra_args: &[&'static str]) {
    ...
    icedragon_cmd(&dir, "cmake", target)
        ...
        .success();
    icedragon_cmd(&dir, "cmake", target)
        .args(["--build", &format!("build-{target}")])
        .assert()
        .success();
    let bin_filename = format!("clang_rt.crtbegin-{arch}.o");
    let bin_path = dir
        .join(format!("build-{target}"))
        .join("lib/linux")
        .join(bin_filename);
    assert_bin_arch(bin_path, elf_machine);
}
