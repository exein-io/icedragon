    ...
    P: AsRef<Path> + fmt::Debug,
{
    let bin_file = File::open(&bin_path)
        .unwrap_or_else(|_| panic!("failed to open the binary file {bin_path:?}"));
    let mut bin_content = Vec::new();
    let mut reader = BufReader::new(bin_file);
    reader
        .read_to_end(&mut bin_content)
        .unwrap_or_else(|_| panic!("could not read the binary file {bin_path:?}"));
    let elf = Elf::parse(&bin_content)
        .unwrap_or_else(|_| panic!("could not parse the binary file {bin_path:?}"));
