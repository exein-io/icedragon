    ...
    P: AsRef<Path>,
{
    ...
    if let Ok(container_image) = env::var("ICEDRAGON_CONTAINER_IMAGE") {
        cmd.args(["--container-image", &container_image]);
    }
    if let Ok(container_tag) = env::var("ICEDRAGON_CONTAINER_TAG") {
        cmd.args(["--container-tag", &container_tag]);
    }
    cmd.arg("--");
    cmd
}
...
fn check_bin_arch<P>(bin_path: P, elf_machine: u16)
where
    P: AsRef<Path>,
{
    let bin_file = File::open(bin_path).expect("failed to open the binary file");
    let mut bin_content = Vec::new();
    let mut reader = BufReader::new(bin_file);
    reader
        .read_to_end(&mut bin_content)
        .expect("could not read the binary file");
    let elf = Elf::parse(&bin_content).expect("could not parse the binary file");
...
fn test_cargo_pulsar(target: &str, elf_machine: u16) {
    ...
    check_bin_arch(bin_path, elf_machine);
}
