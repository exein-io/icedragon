...
fn run_container(
    ...
    unshare(CloneFlags::CLONE_NEWNS | CloneFlags::CLONE_NEWUSER).unwrap();
    mount_volumes(rootfs_dir, volumes)?;
    chroot(rootfs_dir).unwrap();
    ...
    command.envs(prepare_env(triple));
    command.spawn()?.wait()?;
