...
fn prepare_env(triple: &Triple, override_cc_with_cross: bool) -> Vec<(String, String)> {
    ...
    // filter out `RUSTUP_HOME` and variables prefixed by `CARGO` to make sure
    // that the Rust toolchain inside the container is isolated, expecially
    // when icedragon is launched with `cargo run`. Variables prefixed by `SSL`
    // are filtered to make sure that ca-certificates from the container are used.
    let mut env: Vec<(String, String)> = env::vars()
        .filter_map(|(key, value)| {
            if matches!(
                key.as_str(),
                "HOME" | "OLDPWD" | "PATH" | "PWD" | "RUSTUP_HOME" | "USER"
...
fn bootstrap_rustup(envs: &[(String, String)]) -> anyhow::Result<()> {
