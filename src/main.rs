...
fn prepare_env(triple: &Triple) -> Vec<String> {
    ...
    env.extend_from_slice(&[
        // Tell cargo to build for the target.
        format!("CARGO_BUILD_TARGET={triple}"),
        // Use cross clang wrappers.
        format!("CC={triple}-clang"),
        format!("CXX={triple}-clang++"),
        // Make sure LLVM libc++ is used as a standard library.
        format!("CXXFLAGS={} --stdlib=libc++", env::var("CXXFLAGS").unwrap_or_default()),
        // Make sure that:
        // * lld is used as a linker
        // * compiler-rt is used as a runtime library
        // * LLVM libunwind is used as an unwinder
    ...
    // If the target CPU architecture is foreign, use the user-space emulator
    // (QEMU) as a runner for cargo tests.
...
fn container_spec(
    ...
    // Build and save the container spec.
